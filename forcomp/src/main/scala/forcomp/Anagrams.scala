...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    w.toLowerCase.toList.groupBy(e => e).map{
      case (letter, occurences) => letter -> occurences.length}.toList.sortBy(_._1)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yM = y.toMap
    val res = for (xi <- x) yield (xi._1, xi._2 - yM.getOrElse(xi._1, 0))
    res.filter( e => e._2 != 0 )
  }
